  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 2fr 140px 1fr", gap: 10 }}>
          <input value={form.nome} onChange={(e) => setForm((s) => ({ ...s, nome: e.target.value }))} placeholder="Nome do projeto" className="crono-input" />
          <input value={form.cliente} onChange={(e) => setForm((s) => ({ ...s, cliente: e.target.value }))} placeholder="Cliente" className="crono-input" />
          <input value={form.cor} onChange={(e) => setForm((s) => ({ ...s, cor: e.target.value }))} placeholder="#0a4723" className="crono-input" />
          <button className="crono-btn" onClick={handleCriar} disabled={!user || !form.nome.trim()}>
            Criar
          </button>
        </div>

        <textarea value={form.descricao} onChange={(e) => setForm((s) => ({ ...s, descricao: e.target.value }))} placeholder="Descrição (opcional)" className="crono-input" style={{ minHeight: 72 }} />
      </div>

      <div style={{ marginTop: 16, display: "grid", gap: 12 }}>
        {items.map((p) => (
          <div key={p.id} style={{ padding: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.18)", background: p.arquivado ? "rgba(255,255,255,0.04)" : "transparent" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 800 }}>{p.nome || "(sem nome)"}</div>
                <div style={{ opacity: 0.8, fontSize: 12 }}>{p.cliente || "—"} · {p.status || "ativo"}</div>
              </div>
              <div style={{ width: 18, height: 18, borderRadius: 7, background: p.cor || "#0a4723", border: "1px solid rgba(255,255,255,0.25)" }} />
            </div>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
              {!p.arquivado ? (
                <button className="crono-btn" onClick={() => arquivarProjeto(p.id).catch((e) => setErro(String(e?.message || e)))}>
                  Concluir/Arquivar
                </button>
              ) : (
                <button className="crono-btn" onClick={() => desarquivarProjeto(p.id).catch((e) => setErro(String(e?.message || e)))}>
                  Reabrir
                </button>
              )}
